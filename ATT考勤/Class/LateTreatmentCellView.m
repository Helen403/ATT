...
@interface LateTreatmentCellView()
...
@implementation LateTreatmentCellView
...
-(void)setLateTreatmentModel:(LateTreatmentModel *)lateTreatmentMode{
    if (!lateTreatmentMode) {
        return;
    }
    _lateTreatmentModel = lateTreatmentMode;
    if ([LSCoreToolCenter PureLetters:lateTreatmentMode.userName]) {
        self.name.text = lateTreatmentMode.userName;
    }else{
        if (lateTreatmentMode.userName.length==3) {
            self.name.text = [lateTreatmentMode.userName  substringFromIndex:1];
        }else{
            self.name.text = lateTreatmentMode.userName;
        }
    }
