...
@implementation BusinessTravelView

#pragma mark system
-(instancetype)initWithViewModel:(id<HViewModelProtocol>)viewModel{
    
    self.businessTraveViewModel = (BusinessTravelViewModel *)viewModel;
...
-(void)h_bindViewModel{
    [[self.businessTraveViewModel.tableViewSubject takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNumber *x) {
        
        dispatch_sync(dispatch_get_main_queue(), ^{
            self.tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH*0.8, [self h_w:40]*(self.businessTraveViewModel.arr.count+1));
...
-(void)ProveView:(NSNotification*) notification{
    ...
    NSString *companyCode =  [[NSUserDefaults standardUserDefaults] objectForKey:@"companyCode"];
    self.businessTraveViewModel.companyCode = companyCode;
    self.businessTraveViewModel.flowTypeName = @"outWork";
    self.businessTraveViewModel.stepUserCodes= self.stepUserCodes;
    self.businessTraveViewModel.stepUserNames = self.stepUserNames;
    [self.businessTraveViewModel.flowTemplateCommand execute:nil];
}
...
-(BusinessTravelViewModel *)businessTraveViewModel{
    if (!_businessTraveViewModel) {
        _businessTraveViewModel = [[BusinessTravelViewModel alloc] init];
    }
    return _businessTraveViewModel;
}
...
-(void)finish:(UIButton *)button{
    ...
    self.businessTraveViewModel.applyEndDatetime = self.lateTimeShowText.text;
    self.businessTraveViewModel.applyLenHours = [NSString stringWithFormat:@"%f",[LSCoreToolCenter getDifferenceTime:self.applyTimeShowText.text endTime:self.lateTimeShowText.text]];
    
    self.businessTraveViewModel.applyReason = self.textView.text;
    self.businessTraveViewModel.applyStatus = @"0";
    self.businessTraveViewModel.flowInstanceId = self.flowInstanceId;
    self.businessTraveViewModel.cuserCode = self.cuserCode;
    ...
    self.businessTraveViewModel.applyUserCode = user.userCode;
    self.businessTraveViewModel.applyUserName = user.userNickName;
    
    [self.businessTraveViewModel.applyOutWorkCommand execute:nil];
}
...
-(UITableView *)tableView{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BusinessTravelCellView class] forCellReuseIdentifier:[NSString stringWithUTF8String:object_getClassName([BusinessTravelCellView class])]];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return self.businessTraveViewModel.arr.count;
}

#pragma mark tableViewDataSource
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    BusinessTravelCellView *cell = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithUTF8String:object_getClassName([BusinessTravelCellView class])] forIndexPath:indexPath];
    
    cell.businessTravelModel = self.businessTraveViewModel.arr[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BusinessTravelModel *businessTravelModel =  self.businessTraveViewModel.arr[indexPath.row];
    self.compensateShowText.text = businessTravelModel.workName;
    self.workLsh = businessTravelModel.workLsh;
