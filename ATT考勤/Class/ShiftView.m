...
@interface ShiftView()<UITableViewDelegate,UITableViewDataSource>
...
-(instancetype)initWithViewModel:(id<HViewModelProtocol>)viewModel{
    ...
    self.shiftViewModel = (ShiftViewModel *)viewModel;
...
-(void)h_loadData{
    ...
    self.shiftViewModel.companyCode = companyCode;
    [self.shiftViewModel.refreshDataCommand execute:nil];
    
}


-(void)h_bindViewModel{
    [[self.shiftViewModel.tableViewSubject takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNumber *x) {
        
        dispatch_sync(dispatch_get_main_queue(), ^{
            self.tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH*0.8, [self h_w:40]*(self.shiftViewModel.arr.count+1));
            ...
            self.compensateShowText.text = shift.shiftName;
            self.shiftOldLsh = shift.shiftLsh;
            self.shiftNewLsh = shift.shiftLsh;
...
-(void)ProveView:(NSNotification*) notification{
    ...
    NSString *companyCode =  [[NSUserDefaults standardUserDefaults] objectForKey:@"companyCode"];
    self.shiftViewModel.companyCode = companyCode;
    self.shiftViewModel.flowTypeName = @"changeWork";
    self.shiftViewModel.stepUserCodes= self.stepUserCodes;
    self.shiftViewModel.stepUserNames = self.stepUserNames;
    [self.shiftViewModel.flowTemplateCommand execute:nil];
}
...
-(ProveView *)proveView{
    if (!_proveView) {
        ...
        _proveView.flowType = @"changeWork";
    }
    return _proveView;
}
...
-(void)finish:(UIButton *)button{
   ...
   NSInteger a = [LSCoreToolCenter dateTimeDifferenceWithStartTime:self.applyTimeShowText.text endTime:self.lateTimeShowText.text];
    ...
    NSString *companyCode =  [[NSUserDefaults standardUserDefaults] objectForKey:@"companyCode"];
    self.shiftViewModel.companyCode = companyCode;
    self.shiftViewModel.applyStartDatetime = self.applyTimeShowText.text;
    self.shiftViewModel.applyEndDatetime = self.lateTimeShowText.text;
    self.shiftViewModel.applyLenHours = [NSString stringWithFormat:@"%f",[LSCoreToolCenter getDifferenceTime:self.applyTimeShowText.text endTime:self.lateTimeShowText.text]];
    
    self.shiftViewModel.applyReason = self.textView.text;
    self.shiftViewModel.applyStatus = @"0";
    self.shiftViewModel.flowInstanceId = self.flowInstanceId;
    self.shiftViewModel.cuserCode = self.cuserCode;
    ...
    self.shiftViewModel.shiftNewLsh = self.shiftNewLsh;
    self.shiftViewModel.shiftNewName = self.compensateShowText.text;
    
    
    self.shiftViewModel.workLsh = self.workLsh;
    self.shiftViewModel.workName = self.sureTimeShowText.text;
    UserModel *user =  getModel(@"user");
    self.shiftViewModel.applyUserCode = user.userCode;
    self.shiftViewModel.applyUserName = user.userNickName;
    
    [self.shiftViewModel.applyChangeWorkCommand execute:nil];
}
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return self.shiftViewModel.arr.count;
}

#pragma mark tableViewDataSource
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    
    ShiftCellView *cell = [tableView dequeueReusableCellWithIdentifier:[NSString stringWithUTF8String:object_getClassName([ShiftCellView class])] forIndexPath:indexPath];
    
    cell.shiftModel = self.shiftViewModel.arr[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ShiftModel *shift = self.shiftViewModel.arr[indexPath.row];
    if (self.index == 1) {
        self.sureTimeShowText.text = shift.shiftName;
        self.shiftOldLsh = shift.shiftLsh;
    }else{
