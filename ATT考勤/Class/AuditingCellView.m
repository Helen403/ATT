...
@implementation AuditingCellView
...
-(void)setAuditingModel:(AuditingModel *)auditingModel{
    if (!auditingModel) {
        return;
    }
    _auditingModel = auditingModel;

    self.Img.image = ImageNamed(auditingModel.imgtext);
    self.title.text = auditingModel.title;
    self.number.text = auditingModel.number;
