...
-(void)setWeekModel:(WeekModel *)weekModel{
    if (!weekModel) {
        return;
    }
    _weekModel = weekModel;
    self.time.text = weekModel.week;
 
    self.shift.text = [NSString stringWithFormat:@"%@-%@",  [LSCoreToolCenter getFormatter:weekModel.startDate],[LSCoreToolCenter getFormatter:weekModel.endDate]];
    Boolean flag = NO;
    if (![weekModel.earlyWorkCount isEqualToString:@"0"]) {
        self.earlyWork.text = [NSString stringWithFormat:@"本周有%@次早退",weekModel.earlyWorkCount] ;
        flag = YES;
    }else{
        self.earlyWork.text = @"";
    }
    
    if (![weekModel.forgetWorkCount isEqualToString:@"0"]) {
        
        self.forgetWork.text = [NSString stringWithFormat:@"本周有%@次忘记打卡",weekModel.forgetWorkCount] ;
        flag = YES;
    }else{
        self.forgetWork.text = @"";
    }
    
    if (![weekModel.lateWorkCount isEqualToString:@"0"]) {
        
        self.lateWork.text = [NSString stringWithFormat:@"本周有%@次迟到",weekModel.lateWorkCount] ;
        flag = YES;
    }else{
        self.lateWork.text = @"";
    }
    
    if (![weekModel.goOutWorkCount isEqualToString:@"0"]) {
        self.goOutWork.text = [NSString stringWithFormat:@"本周有%@次外出",weekModel.goOutWorkCount];
        flag = YES;
    }else{
        self.goOutWork.text = @"";
    }
    
    if (![weekModel.offWorkCount isEqualToString:@"0"]) {
        self.offWork.text = [NSString stringWithFormat:@"本周有%@次请假",weekModel.offWorkCount] ;
        flag = YES;
    }else{
        self.offWork.text = @"";
    }
    ...
    if (![weekModel.outWorkCount isEqualToString:@"0"]) {
        self.outWork.text = [NSString stringWithFormat:@"本周有%@次出差",weekModel.outWorkCount] ;
        flag = YES;
    }else{
        self.outWork.text = @"";
    }
    
    if (![weekModel.overWorkCount isEqualToString:@"0"]) {
        self.overWork.text = [NSString stringWithFormat:@"本周有%@次加班",weekModel.overWorkCount] ;
        flag = YES;
    }else{
        self.overWork.text = @"";
    }
