...
@implementation AlreadyDetailsView

#pragma mark system
-(instancetype)initWithViewModel:(id<HViewModelProtocol>)viewModel{
    
    self.alreadyDetailsViewModel = (AlreadyDetailsViewModel *)viewModel;
...
-(void)h_bindViewModel{
    ...
    [[self.alreadyDetailsViewModel.tableViewSubject takeUntil:self.rac_willDeallocSignal] subscribeNext:^(NSNumber *x) {
        dispatch_sync(dispatch_get_main_queue(), ^{
            
            AlreadyTreatmentModel * lateTreatment = self.alreadyDetailsViewModel.lateArr[self.indexTmp];
            ...
            if ([@"请假" isEqualToString:apType]) {
                //=================================================
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"请假类型:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                 self.compensate.text = [NSString stringWithFormat:@"时长:%.f小时",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name.floatValue/60.0f];
                // =================================================
            }
            if ([@"加班" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"加班类型:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = [NSString stringWithFormat:@"补偿类型:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name];
                // =================================================
            }
            if ([@"出差" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"出差地点:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = [NSString stringWithFormat:@"费用类型:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name];
                // =================================================
            }
            if ([@"外出" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"外出地点:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = [NSString stringWithFormat:@"外出类型:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name];
                // =================================================
            }
            if ([@"换班" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"原来班次:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = [NSString stringWithFormat:@"新的班次:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name];
                // =================================================
            }
            ...
            if ([@"辞职" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"申请时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"计划离岗:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"辞职形式:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
            ...
            if ([@"调休" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"开始时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"结束时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"调休类别:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = @"";
                // =================================================
            }
            if ([@"漏打卡" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"漏打时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"补打时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                if (self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name==nil) {
                    self.witness.text = @"证明人:";
                }else{
                    self.witness.text = [NSString stringWithFormat:@"证明人:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                }
                self.compensate.text = @"";
                // =================================================
            }
            if ([@"消迟到" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"迟到时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"正常时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                if (self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name==nil) {
                    self.witness.text = @"证明人:";
                }else{
                    self.witness.text = [NSString stringWithFormat:@"证明人:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
            ...
            if ([@"消早退" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"早退时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"正常时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                if (self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name==nil) {
                    self.witness.text = @"证明人:";
                }else{
            ...
            if ([@"报销" isEqualToString:apType]) {
                ...
                self.startTime.text=[NSString stringWithFormat:@"申请时间:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.startDate];
                self.endTime.text = [NSString stringWithFormat:@"报销部门:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.endDate];
                self.witness.text = [NSString stringWithFormat:@"费用类别:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type1Name];
                self.compensate.text = [NSString stringWithFormat:@"报销金额:%@",self.alreadyDetailsViewModel.alreadyDetailsModel.type2Name];
                // =================================================
            }
            
            self.reason.text = self.alreadyDetailsViewModel.alreadyDetailsModel.reason;
            self.page.text = [NSString stringWithFormat:@"第%ld页/共%lu页",(long)self.indexTmp+1,(unsigned long)self.alreadyDetailsViewModel.lateArr.count];
            
            self.view.arr = self.alreadyDetailsViewModel.arr;
        });
...
-(void)last:(UIButton *)button{
    ++self.indexTmp;
    if (self.indexTmp==self.alreadyDetailsViewModel.lateArr.count) {
        self.indexTmp=self.alreadyDetailsViewModel.lateArr.count-1;
    }
